...
%\captionsetup{labelformat=empty}
%\caption{Modèle}
  \includegraphics[width=.8\textwidth]{ann_01}
\end{center}
%\end{figure}

\subsubsection*{Paramétrage 1 : modèle avec efforts ponctuels entre le sol et la foreuse}
\begin{itemize}
  \item $F_{g} \vect{z}$, inconnu, l'effort du sol $\mathbf{0}$ sur $\mathbf{S 1}$, supposé ponctuel au centre $I$ de la surface de contact entre la chenille gauche $\text{cg}$ et le sol tel que $\|\overrightarrow{OI}\|=a=\SI{2,1}{m}$;
  \item $F_{d} \vec{z}$, inconnu, l'effort du sol $\mathbf{0}$ sur $\mathbf{S 1}$, supposé ponctuel au centre $J$ de la surface de contact entre la chenille droite $\text{cd}$ et le sol tel que $\|\overrightarrow{OJ}\|=a=\SI{2,1}{m}$.
\end{itemize}
...
\begin{itemize}
  ...
  \item $p_{g}(y)=A \dfrac{y}{L}+B$, la pression du sol 0 sur la chenille gauche $\text{cg}$ au point $\mathrm{P}(x, y, 0)$ où $A$ et $B$, homogènes à des pressions, sont inconnues et à déterminer ;
  \item $p_{d}(y)=C \dfrac{y}{L}+D$, la pression du sol 0 sur la chenille droite $\text{cd}$ au point $\mathrm{P}(x, y, 0)$ où $C$ et $D$, homogènes à des pressions, sont inconnues et à déterminer ;
...
%\begin{center}
%\includegraphics[width=.9\textwidth]{2025_10_26_292d8f726e32404b86d3g-18}
%\end{center}
%
%
%%\section*{ANNEXE 5 -- Schéma-bloc ：modélisation de l'asservissement}
