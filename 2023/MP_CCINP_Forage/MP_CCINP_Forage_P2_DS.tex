...
On pose le changement de variables suivant : $z_{5}(t)=\hat{z}_{5}(t)-\dfrac{\Delta z_{e}}{2}$ et $z_{4}(t)=\hat{z}_{4}(t)+\dfrac{\Delta z_{e}}{2}$, permettant de s'affranchir des actions de la pesanteur. On rappelle que $\Delta z_{e}$ est constant.
\fi
...
De plus, $v(t)=\dfrac{\dd z_{4}(t)}{\dd t}$, soit $Z_{4}(p)=\dfrac{V(p)}{p}$.\\
On obtient $\Delta F_{w}(p)=\dfrac{k_{T}+\lambda_{T} p+M_{T} p^{2}}{p} V(p)-\left(k_{T}+\lambda_{T} p\right) Z_{5}(p)$

On en déduit $H_{8}(p)=\dfrac{k_{T}+\lambda_{T} p+M_{T} p^{2}}{p}$\\
La deuxième équation donne : $Z_{5}(p)=\dfrac{1}{k_{T}+\lambda_{T} p+M_{K} p^{2}}\left[\left(k_{T}+\lambda_{T} p\right) \dfrac{V(p)}{p}+F_{\text {sol }}(p)\right]$.\\
On en déduit $H_{6}(p)=\dfrac{k_{T}+\lambda_{T} p}{p}$\\
Et, en replaçant $Z_{5}(p)$ dans (i) : $H_{7}(p)=\dfrac{k_{T}+\lambda_{T} p}{k_{T}+\lambda_{T} p+M_{K} p^{2}}$
\end{corrige}
\else
\fi
...
\ifprof
 ...
 On obtient:
$Q_{p}(p)=K_{s} U_{s}(p)$,
$\dfrac{V_{0}}{B} p \Delta P(p)=Q_{p}(p)-Q_{m}(p)$,
$Q_{m}(p)=C_{Y} \Omega_{m}(p)$,
$C_{m}(p)=C_{Y} \Delta P(p)$,

$\left(a+\indice{J}{eq} p\right) \Omega_{m}(p)=C_{m}(p)-C_{r}(p)$
...
\ifprof
 ...
 \textbf{UPSTI} --  
$H_{1}(p)=K_{s} $,
$H_{2}(p)=\dfrac{B}{V_{0} p}$,
$H_{3}(p)=C_{Y}$,
$H_{4}(p)=C_{Y}$,

$H_{5}(p)=\dfrac{1}{a+\indice{J}{eq} p}$.
\end{corrige}
\else
\fi
...
\ifprof
 ...
 $\indice{K}{ihm}$ permet de s'assurer qu'en régime permanent, l'image de l'erreur $\varepsilon_{V}$ est nulle lorsque l'erreur est nulle, qu'elle que soit la consigne.

Condition vérifiée si $\indice{K}{ihm}=\indice{K}{gen} \dfrac{2}{R_{2}}$.
\end{corrige}
\else
\fi
...
\ifprof
 ...
 \textbf{UPSTI} --  
 $$
\dfrac{\Omega_{m}(p)}{U_{S}(p)}=H_{1}(p) \dfrac{H_{2}(p) H_{3}(p) H_{5}(p)}{1+H_{2}(p) H_{3}(p) H_{4}(p) H_{5}(p)}
$$
...
\ifprof
 ...
 \textbf{UPSTI} --  
 $$
G_{B O}(p)=\dfrac{K_{0} K_{p} K_{h}}{1+\dfrac{2 z_{h}}{\omega_{h}} p+\dfrac{p^{2}}{\omega_{h}^{2}}}
...
\ifprof
 \begin{corrige}
 \textbf{UPSTI} --  
 $C(p)=K_{p} \dfrac{1+T_{i} p}{p}$ et $G_{B O}(p)=\dfrac{K_{0} K_{p} K_{h}\left(1+T_{i} p\right)}{p\left(1+\dfrac{2 z_{h}}{\omega_{h}} p+\dfrac{p^{2}}{\omega_{h}^{2}}\right)}$, d'ordre 3 et de classe 1.\\
...
\ifprof
 \begin{corrige}
 \textbf{UPSTI} --  
 Avec $K_{p}=1$ et $T_{i}=10 \mathrm{~s}, C(p)=\dfrac{1}{10 p}(1+10 p)$.\\
