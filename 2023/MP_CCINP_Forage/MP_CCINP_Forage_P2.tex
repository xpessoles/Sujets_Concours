...
%\begin{equation*}
$\dfrac{V_{0}}{B} \cdot \dfrac{d \Delta p(t)}{d t}=q_{p}(t)-q_{m}(t)\quad \text{(ii)} $%\tag{ii}$
%\end{equation*}
\end{itemize}
...
\begin{itemize}
  \item L'équation du mouvement du mécanisme s'écrit : $\indice{J}{eq}  \dfrac{\d \omega_{m}(t)}{\d t}+a \omega_{m}(t)=C_{m}(t)-C_{r}(t) \quad \text{(v)}$
...
%Q24. 
\question{\label{2023_MP_CCINP_q24}À partir de la figure \ref{2023_MP_CCINP_fig_11}, exprimer la fonction de transfert $\frac{\Omega_{m}(p)}{U_{s}(p)}$, notée $H(p)$, à partir des fonctions de transfert $H_{j}(p)$, où $j \in\{1 ; 2 ; 3 ; 4 ; 5\}$.}
