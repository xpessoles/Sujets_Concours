...
\question{Dans cette question uniquement, on considère le système formé par les pièces 0, 3, 4 et 5. Déterminer degré d'hyperstatisme du modèle. S'il est hyperstatique, proposé une ou plusieurs modifications de liaisons pour le rendre isostatique.}
\ifprof
\begin{corrige}
\end{corrige}
\else
...
% QUESTION SUPPLEMENTAIRE
\question{Déterminer degré d'hyperstatisme du modèle complet.}
\ifprof
\begin{corrige}
\end{corrige}
\else
...
Proposer une stratégie (isolement, théorèmes utilisé, point d'application si nécessaire, projection) permettant d'établir cette relation. On s'appuiera sur un graphe de structure.}
%Montrer également que cette relation est indépendante de la longueur $L_5$ et expliquer l'avantage technique de ce résultat. }
