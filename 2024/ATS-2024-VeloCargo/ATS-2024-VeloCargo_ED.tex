...
\section*{2 Effort de mise en stationnement}
...
\begin{figure}[!htb]
\begin{center}
\includegraphics[max width=\textwidth, center]{2024_12_06_8b2ce2e701dae8972925g-02}
...
\subsection*{2.1.1 Centre de gravité \(G_{V}\) du vélo à vide}
...
Mouvement du vélo On suppose dans cette partie que la fourche avant est rigide durant la phase de béquillage. Dès lors, la roue avant se décollera du sol sous l'action de traction du vélo par l'usager tandis que la roue arrière reculera.
...
\begin{figure}[!htb]
\begin{center}
\includegraphics[width=0.8\textwidth]{images/2024_12_06_8b2ce2e701dae8972925g-04(1)}
\end{itemize}
...
Figure \ref{fig_2_cargo}.3 - Paramétrage utilisé pour l'étude du béquillage
...
Valeurs limites de \(\alpha\) On note \(\alpha_{i}\) et \(\alpha_{f}\left(\alpha_{i}<\alpha_{f}\right)\) les valeurs de l'angle \(\alpha\) entre lesquelles la roue avant est décollée du sol.
...
\begin{figure}[!htb]
\begin{center}
\includegraphics[max width=\textwidth, center]{2024_12_06_8b2ce2e701dae8972925g-04}
...
\(\)
F_{C} \approx \frac{\mu \cdot Y_{04}-(\mu-k) \cdot m_{T} \cdot g}{y_{M}+a}
\(\)
}
...
\(\)
F_{C}=\frac{f \cdot k \cdot m_{T} \cdot g}{\mu+f \cdot\left(y_{M}+a\right)}
\(\)

On estime alors l'effort minimal pour assurer le béquillage en charge maximale à \(F_{C} \approx 80 \mathrm{~N}\).

\question{Conclure sur le respect de l'exigence 110 du cahier des charges du vélo (figure 2.1, page 4).}

\section*{3 Vérification du dimensionnement des organes participant au freinage}
La sécurité des utilisateurs du vélo cargo passe par un freinage performant. Les pneumatiques et les freins doivent garantir une distance d'arrêt du vélo raisonnable même lorsque celui-ci est chargé au maximum. L'étude proposée dans cette partie vise à vérifier le dimensionnement de ces éléments à la lumière du cahier des charges précisé dans le diagramme des exigences de la figure 3.1.\\[0pt]
req [] Freinage [Urgence]
...
\begin{tabular}{l}
\(\mathrm{Id}=\) "200" \\
Text \(=\) "Le vélo cargo en charge doit pouvoir réaliser un arrêt d'urgence dans des conditions de sécurité optimales." \\
\hline
\end{tabular} &  \\
\hline
\end{tabular}
...
\end{tabular} \\
\hline
\( \begin{aligned} & \mathrm{Id}=" 210 " \\ & \text { Text="La décélération doit pouvoir } \\ & \text { atteindre } 4.5 \mathrm{~m} / \mathrm{s}^{2} " \end{aligned} \) & \begin{tabular}{l}
\( \mathrm{Id}=" 220 " \) \\
Text="La distance d'arrêt pour une vitesse initiale de \(25 \mathrm{~km} / \mathrm{h}\) ne doit pas excéder 6 m ." \\
\end{tabular} \\
\hline
\end{tabular}
\end{center}

Figure \ref{fig_3_cargo}.1 - Extrait du cahier des charges relatif au freinage d'urgence

\subsection*{3.1 Modèle d'étude}
...
où \(f^{\prime}\) désigne, selon la configuration, le coefficient de frottement ou d'adhérence du contact roue-sol (supposés égaux). La valeur de \(f^{\prime}\) dépend de nombreux facteurs parmi lesquels l'état du pneumatique et l'humidité de la chaussée (voir tableau 3.3, page 10).
...
Tablead 3.2 - Caractéristiques inertielles des ensembles pour l'étude du freinage
...
Tableau 3.3 - Valeurs du coefficient de frottement/d'adhérence (supposés égaux) \(f^{\prime}\) en fonction de l'état du pneu et de la route

\subsection*{3.1.3 Grandeurs cinématiques}
Vitesses de pivotement des roues On note \(\omega_{12}\) et \(\omega_{32}\) les vitesses de rotation telles que :

\(\)
\overrightarrow{\Omega(1 / 2)}=\omega_{12} \cdot \vec{z} \quad \text { et } \quad \overrightarrow{\Omega(3 / 2)}=\omega_{32} \cdot \vec{z}
\(\)

\section*{Position, vitesse et accélération du point \(G_{2}\) par rapport au sol}
— La position selon \(\vec{x}\) du point \(G_{2}\) dans le repère du sol (0) est notée \(x_{G_{2}}: x_{G_{2}}(t)=\overrightarrow{O G_{2}} \cdot \vec{x}\).

\begin{itemize}
  \item La vitesse horizontale de \(G_{2}\) est notée \(v_{G_{2}}: v_{G_{2}}(t)=\frac{\mathrm{d} x_{G_{2}}(t)}{\mathrm{d} t}\).
  \item L'accélération horizontale du point \(G_{2}\) est désignée par \(\gamma_{G_{2}}: \gamma_{G_{2}}(t)=\frac{\mathrm{d} v_{G_{2}}(t)}{\mathrm{d} t}\)
\end{itemize}

\subsection*{3.2 Capacité de freinage d'urgence}
\section*{Objectif}
Déterminer les conditions portant sur les pneumatiques qui permettent d'assurer un freinage d'urgence conforme aux exigences de la figure 3.1.
...
\(\)
X_{01}=\frac{C_{f 1}+\frac{I_{B 1}}{R_{1}} \gamma_{G_{2}}}{R_{1}} \quad \text { et } \quad X_{03}=\frac{C_{f 3}+\frac{I_{C 3}}{R_{3}} \gamma_{G_{2}}}{R_{3}}
\(\)
}
...
\(\)
Y_{03}=\frac{1}{L}\left[g\left(m_{2} \cdot x_{2}+m_{3} \cdot L\right)-\gamma_{G_{2}}\left(y_{2} \cdot m_{2}+R_{1} \cdot m_{1}+R_{3} \cdot m_{3}\right)+I_{B 1} \cdot \frac{\mathrm{~d} \omega_{12}}{\mathrm{~d} t}+I_{C 3} \cdot \frac{\mathrm{~d} \omega_{32}}{\mathrm{~d} t}\right]
\(\)
}
...
\(\)
\begin{array}{ll}
X_{01}=\frac{R_{1} \cdot C_{f 1}+I_{B 1} \cdot \gamma_{G_{2}}}{R_{1}^{2}} & Y_{01}=\frac{1}{L}\left[g\left(m_{2} \cdot\left(L-x_{2}\right)+m_{1} \cdot L\right)+\gamma_{G_{2}}\left(y_{2} \cdot m_{2}+R_{1} \cdot m_{1}+R_{3} \cdot m_{3}+\frac{I_{B 1}}{R_{1}}+\frac{I_{C 3}}{R_{3}}\right)\right] \\
X_{03}=\frac{R_{3} \cdot C_{f 3}+I_{C 3} \cdot \gamma_{G_{2}}}{R_{3}^{2}} & Y_{03}=\frac{1}{L}\left[g\left(m_{2} \cdot x_{2}+m_{3} \cdot L\right)-\gamma_{G_{2}}\left(y_{2} \cdot m_{2}+R_{1} \cdot m_{1}+R_{3} \cdot m_{3}+\frac{I_{B 1}}{R_{1}}+\frac{I_{C 3}}{R_{3}}\right)\right]
\end{array}
\(\)
...
%\section*{4 Étude du capteur de couple}
%
...
%\begin{figure}[!htb]
%\begin{center}
%\includegraphics[width=0.8\textwidth]{images/2024_12_06_8b2ce2e701dae8972925g-12(1)}
%\caption{Extrait du cahier des charges relatif à la mesure du couple appliqué par l'utilisateur sur le pédalier. \label{fig7}}
%\end{center}
%\end{figure}
%
%\subsection*{4.1 Présentation du capteur}
...
%\begin{figure}[!htb]
%\begin{center}
%\includegraphics[width=0.8\textwidth]{images/2024_12_06_8b2ce2e701dae8972925g-12}
%\caption{Architecture du capteur de couple \label{fig8}}
%\end{center}
%\end{figure}
%
%Constitution du transducteur à reluctance variable Un schéma de principe du transducteur à reluctance variable autour duquel est construit le capteur de couple étudié est donné figure 4.3. Ce transducteur est constitué de quatre ensembles :
%
%\begin{itemize}
...
%\begin{figure}[!htb]
%\begin{center}
%\includegraphics[width=0.8\textwidth]{images/2024_12_06_8b2ce2e701dae8972925g-13}
%\caption{Principaux éléments du transducteur de couple \label{fig9}}
%\end{center}
%\end{figure}
%
%\subsection*{4.2 Tension de sortie du transducteur}
%\section*{Objectif}
...
%Le transducteur de couple est modélisé par deux bobines d'inductances respectives \(L_{1}\) (bobine de compensation) et \(L_{1}+\Delta L\) (bobine de détection). La résistance de ces bobines n'est pas négligée. Ces bobines sont insérées dans le montage schématisé figure 4.4. Les deux sources de tension sinusoïdales \(v_{1}\) et \(v_{2}\) ont même valeur moyenne, même amplitude et sont en opposition de phase. La différence de potentiel \(v_{s}\) entre les points \(A\) et \(B\) du schéma de la figure 4.4 constitue la tension de sortie du transducteur.
%
%Régime continu On étudie dans un premier temps le fonctionnement du capteur en régime continu.
%
%\fi
...
%\end{enumerate}
%
%Régime alternatif L'étude du régime continu a permis de montrer qu'il est possible de remplacer les deux sources de tension \(v_{1}\) et \(v_{2}\) par une unique de source de tension \(v_{12}=v_{1}-v_{2}\). Le transducteur fonctionnant en régime sinusoïdal forcé, on utilise les notations complexes des grandeurs électriques. À partir de la question 33, l'étude est menée sur la base du schéma de la figure 4.5. Les valeurs numériques définies figure 4.4 sont conservées. La tension
%
%\(\)
...
%\begin{figure}[!htb]
%\begin{center}
%\includegraphics[width=0.8\textwidth]{images/2024_12_06_8b2ce2e701dae8972925g-14}
%\caption{ Modèle électrique du transducteur de couple et grandeurs associées \label{fig4.4}}
%\end{center}
%\end{figure}
%
%\(\)
...
%R_{1} & =5 \Omega \\
%v_{1} & =\frac{E}{2}+\sqrt{2} \cdot A \cdot \cos (2 \cdot \pi \cdot f \cdot t) \\
%v_{2} & =\frac{E}{2}-\sqrt{2} \cdot A \cdot \cos (2 \cdot \pi \cdot f \cdot t) \\
%E & =24 \mathrm{~V} \\
%A & =8 \mathrm{~V} \\
%f & =20 \mathrm{kHz}
%\end{aligned}
...
%\begin{figure}[!htb]
%\begin{center}
%\includegraphics[max width=\textwidth, center]{2024_12_06_8b2ce2e701dae8972925g-14(1)}
...
%
%\(\)
%\underline{V_{S}}=\frac{j \cdot X_{v a r}}{2 \cdot\left(R_{t o t}+j \cdot X_{t o t}\right)} \cdot \underline{V_{12}}
%\(\)
%
...
%\begin{figure}[!htb]
%\begin{center}
%\includegraphics[width=0.8\textwidth]{images/2024_12_06_8b2ce2e701dae8972925g-15}
...
%\begin{figure}[!htb]
%\begin{center}
%\includegraphics[width=0.8\textwidth]{images/2024_12_06_8b2ce2e701dae8972925g-16}
%
...
%
%\(\)
%\frac{2^{j}}{2^{16}} \cdot(\text { Vref }+- \text { Vref }-)
%\(\)
%
...
%\begin{figure}[!htb]
%\begin{center}
%\includegraphics[width=0.8\textwidth]{images/2024_12_06_8b2ce2e701dae8972925g-17}
%
%\begin{center}
%\begin{tabular}{lll}
%Grandeur & Symbole & Valeur \\
...
%
%Phase 1\\
%\includegraphics[max width=\textwidth, center]{2024_12_06_8b2ce2e701dae8972925g-18}
%
%Phase 2\\
%\includegraphics[max width=\textwidth, center]{2024_12_06_8b2ce2e701dae8972925g-18(2)}
%
%Phase 3\\
%\includegraphics[max width=\textwidth, center]{2024_12_06_8b2ce2e701dae8972925g-18(1)}
%
%FIGURE 5.2 - FCEM et courants idéaux dans chaque phase de la machine\\
%\includegraphics[max width=\textwidth, center]{2024_12_06_8b2ce2e701dae8972925g-18(3)}
...
%\begin{figure}[!htb]
%\begin{center}
%\includegraphics[width=0.8\textwidth]{images/2024_12_06_8b2ce2e701dae8972925g-20}
...
%\begin{figure}[!htb]
%\begin{center}
%\includegraphics[width=0.8\textwidth]{images/2024_12_06_8b2ce2e701dae8972925g-20(1)}
...
%\begin{figure}[!htb]
%\begin{center}
%\includegraphics[width=0.8\textwidth]{images/2024_12_06_8b2ce2e701dae8972925g-21(1)}
%\end{itemize}
...
%\begin{figure}[!htb]
%\begin{center}
%\includegraphics[width=0.8\textwidth]{images/2024_12_06_8b2ce2e701dae8972925g-21}
...
%\begin{figure}[!htb]
%\begin{center}
%\includegraphics[width=0.8\textwidth]{images/2024_12_06_8b2ce2e701dae8972925g-22}
...
%\else
%\(\)
%H_{G}(p)=\frac{C_{p m}(p)}{\Delta G(p)}
%\(\)
%
...
%
%\(\)
%H_{a s}(p)=\left.\frac{C_{p m}(p)}{C_{p c}(p)}\right|_{C_{p r}=0}
%\(\)
%
...
%
%\(\)
%C(p)=K_{c} \frac{1+T_{i} \cdot p}{T_{i} \cdot p}
%\(\)
%
%Avec ce correcteur, la fonction de transfert \(H_{a s}(p)\) devient :
%
%\(\)
%H_{a s}(p)=\frac{K_{a s} \cdot\left(K_{r}\right)^{2} \cdot J_{e q} \cdot K_{c}-\left(K_{m}\right)^{2} \cdot T_{i}+K_{a s} \cdot\left(K_{r}\right)^{2} \cdot J_{e q} \cdot K_{c} \cdot T_{i} \cdot p}{K_{c} \cdot\left(K_{r}\right)^{2} \cdot J_{e q}+\left(K_{m}\right)^{2} \cdot T_{i}+\left(K_{r}\right)^{2} \cdot J_{e q} \cdot T_{i} \cdot\left[R_{m}+K_{c}\right] \cdot p+\left(K_{r}\right)^{2} \cdot J_{e q} \cdot L_{m} \cdot T_{i} \cdot p^{2}}
%\(\)
%
%Exigence de précision Le respect de l'exigence 420 impose :
%
%\(\)
%\frac{K_{c}}{T_{i}} \geq \frac{100+95 \cdot K_{a s}}{5 \cdot K_{a s}} \cdot \frac{\left(K_{m}\right)^{2}}{K_{r} \cdot J_{e q}} \text { soit : } \quad \frac{K_{c}}{T_{i}} \geq 12.3 \cdot 10^{-3} \frac{\mathrm{~V}}{\mathrm{~A} \cdot \mathrm{~s}} \quad \text { pour } K_{a s}=0.5 \text { (cas le plus sévère) }
%\(\)
%
...
\includegraphics[max width=\textwidth, center]{2024_12_06_8b2ce2e701dae8972925g-25(2)}

DR B : Figures de calcul - Question 4\\
\includegraphics[max width=\textwidth, center]{2024_12_06_8b2ce2e701dae8972925g-25(1)}\\
\includegraphics[max width=\textwidth, center]{2024_12_06_8b2ce2e701dae8972925g-25}

DR C : Évolution de la tension Vcna - Question 41\\
\includegraphics[max width=\textwidth, center]{2024_12_06_8b2ce2e701dae8972925g-26(1)}

DR D : Mailles actives - Question 49\\
\includegraphics[max width=\textwidth, center]{2024_12_06_8b2ce2e701dae8972925g-26}
