...
\ifprof
\begin{corrige}~\\ \vspace{-.4cm}
\begin{enumerate}
\item $R_D$ est toujours positif (poussée d’Archimède) donc $G_x$ suffit à caractériser le signe de $R_D G_x$.

\item A:4, B:3, C:1, D:6, E:5, F:2.
\end{enumerate}
\end{corrige}
\else
\fi
...
\item 
\begin{center}
\includegraphics[width=.4\linewidth]{cor_01}
\end{center}
...
\item Dans le but de chiffrer la valeur maximale de la puissance que doit fournir l’actionneur pour réaliser le mouvement prévu, tracer sur votre copie, à l’aide de l’« Annexe 2 » et de la figure R4 ci-dessous, l’allure de l’évolution temporelle de cette puissance. Pour cela, évaluer les valeurs aux instants $t = \SI{0}{s}$, $t = \SI{1}{s}$,     $t= \SI{3}{s}$ et $t = \SI{4}{s}$.
...
\begin{enumerate} 
\item à \SI{1}{s}, $2200+5800+2500+4000=\SI{14500}{W}$; à \SI{3}{s} $0+4000+2500+16000=\SI{22500}{W}$. Le maximum est à environ \SI{22,5}{kW}.
Le maximum est bien sur cet intervalle car le poids y est résistant (le poids est moteur sur $[\SI{5}{s} ; \SI{8}{s}]$). 
\item La différence est de \SI{7,5}{kW}. Elle ne peut pas provenir des hypothèses faites (liaisons parfaites et $R_N$ galiléen). Elle provient certainement du fait que le système est surdimensionné pour pallier les erreurs de modélisation des actions de l'eau, le vieillissement de la quille avec les algues collées qui rajoutent du poids...
\end{enumerate}
\end{minipage}
\end{corrige}
\else
...
\ifprof
\begin{corrige}
L'effort radial vaut $F=\dfrac{1}{2e}\sqrt{\left((e+d)F_{21}\right)^2+\left(eM_1g\right)^2}=\SI{15,8e4}{N}$.

La pression exercée sur le coussinet est $p=\dfrac{F}{d_cL_c}=\SI{39,62}{N/mm^2}$, ce qui valide le choix fait.
\end{corrige}
\else
\fi
...
\begin{center}
\includegraphics[width=.9\linewidth]{cor_03}
\end{center}
\end{minipage} \hfill
\begin{minipage}[c]{.45\linewidth}
...
\captionof{figure}{Fonctionnement de la servovalve. \label{fig_15_quille}}
\end{center}
\end{minipage} \hfill
\begin{minipage}[c]{.45\linewidth}
Ainsi :
...
\begin{center}
\includegraphics[width=.5\linewidth]{dr_06}
%\caption{Stabilité << de formes >> \label{fig_02_quille}}
\end{center}
\fi
...
\item Donner l’intervalle de temps où le retour d’information a une influence sur la commande du vérin et celui où il n’en a pas. Associer à chacun de ces intervalles le modèle utile : modèle en « boucle fermée » ou en « boucle ouverte ».
\end{itemize}}
...
%(pb sur l'unité de q : je suppose 10-3 m3.s-1) 
...
\begin{corrige}
\begin{enumerate}
\item $v(0+)=\theta_0 K_c'-0 = 5\times 1,1 = \SI{5,5}{V}$.
\item $v(0+) < \SI{10}{V}$ donc le fonctionnement est sans saturation.
\item Sans saturation on valide l'hypothèse de système linéaire.
\end{enumerate}
\end{corrige}
\else
\fi
...
\begin{corrige}
\begin{enumerate}
\item $\omega_0 = \SI{5}{rad/s}$, $20\log\left(\dfrac{\indice{K}{BO}}{\omega_0}\right)=20\log\left(\dfrac{2,2}{5}\right) \simeq -\SI{7}{dB}$.
\begin{center}
\includegraphics[width=.6\linewidth]{cor_04}
\end{center}
%\caption{Stabilité << de formes >> \label{fig_02_quille}}

\item $\dfrac{2\xi}{\omega_0} = 0,12$ $\Rightarrow \xi = \dfrac{0,12}{2}\omega_0 = 0,3$. $\omega_r = \omega_0 \sqrt{1-2\xi^2} =\SI{4,5}{rad/s}$.
\item $\Delta K = 20\log\left( \dfrac{1}{2\xi\sqrt{1-\xi^2}}\right) = \SI{4,84}{dB}$.
\end{enumerate}
\end{corrige}
\else
\fi
...
\begin{figure}[!h]
\centering
\includegraphics[width=.6\linewidth]{fig_16}
...
\end{enumerate}}
\ifprof
\begin{corrige}$| C(j\omega^*)|_{\text{dB}} = 20 \log \indice{K}{Cor} - 10 \log (1+b) + 10\log \left(1+\dfrac{1}{b}\right) = -10\log b$.

\begin{center}
\rotatebox{90}{\includegraphics[width=.5\linewidth]{cor_05}}
\end{center}
...
\begin{corrige}
\begin{enumerate}
\item $10\log b = \Delta K$ soit $b=10^{\Delta K/10}$ et $b=3,05$. Il faut $\omega^* = \omega_r = \dfrac{1}{T\sqrt{b}}$ et $T = \dfrac{1}{\omega_r \sqrt{b}}=\SI{0,127}{s}$.
\item $\varpi(\omega^*) = \arcsin \dfrac{1-b}{1+b} = -\SI{30,4}{\degres}$.
\end{enumerate}
\end{corrige}
\else
\fi
...
\ifprof
\begin{corrige}$\indice{\omega}{max} = \dfrac{3}{0,625}=\SI{4,8}{\degres/s}$.
\begin{center}
\includegraphics[width=.4\linewidth]{cor_06}
\end{center}
