...
de$g\left( X + U_X(X,Y),Y + U_Y(X,Y)\right) $ en $(X,Y)$, donner l’expression de la fonction coût en fonction de $f(X,Y)$, $g(X,Y)$, $\vect{\Delta} g  (X,Y)$, $\alpha_i$ et $\vect{\varphi}_i(X,Y)$ où $\vect{\Delta} g$ désigne le gradient spatial de $g$.}
\ifprof
\begin{corrige}
\end{corrige}
\else
...
%Q
\question{\label{q:22} La figure B du document réponse montre le diagramme de Bode de la fonction $H(p)$. Justifier puis tracer directement
